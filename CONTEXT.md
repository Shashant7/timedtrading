...
- **DCA side effects can die after the lot commits — the 15:50 ET sweep heals them (2026-08-12)**: NVDA 8/11: invocation hard-killed between lot INSERT and ledger/decision/bell/Discord/email/mirror; day-lock + claim correctly block reruns, so recovery is the idempotent `sweepInvestorDcaSideEffects` (cron 15:50 ET, manual `POST /timed/admin/investor/dca-sweep`). Mirror catch-up now takes `trust_fresh_lot_ms` (RTH clock) so thesis gates (`zone_exhausted`) can't veto mirroring a buy the model itself just executed. Forensics tell: bell `created_at == lot.ts` means the daily backfill wrote it, not the live path.
