...
- **ST / LETF entry must reach the broker (2026-09-03)**: UDOW + TQQQ + TJX wrote the model book and never filled. Three bugs: (1) cash scale `Math.floor` rejected a $92 Roth fractional (0.68 TJX / 1.26 UDOW); (2) `forwardOrderToBridge` treated HTTP 200 without `ok:true` as a fill, so catch-up never retried; (3) no trader/index-trend ENTRY catch-up — index-trend is buried in `/timed/options/all`. Fix: `scaleQtyForCeiling({fractional})`, `bridgeResponseIsOk`, buy-side ring requires an order id, `POST /timed/admin/broker-bridge/catchup-trader-entries` + `*/5` RTH cron. Do not auto-enable Individual. Skip SHORT on IRA. Do not chase >5% drift.
