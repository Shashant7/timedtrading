...
- **Weekend CMT Desk / TT Setups (2026-09-12):** `*/5` scoring skips outside OH. Saturday 10:00 ET (tt-research hourly) pages a full-universe rescore, then emails Pro/VIP/Admin a short unique list (story + Daily/4H/Weekly chart + what the model is watching for). Volume is a CMT input. Not Newton's monthly Upticks, not a buy path, not an indicator dump. Refresh without `email=1` must not send. Playbook: `skills/weekend-desk.md`.
