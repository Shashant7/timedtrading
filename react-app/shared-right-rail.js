  ...
  window.TickerDetailRightRailFactory = function (deps) {
    ...
    function InvestorTabPanel({ ticker, latestTicker, effectiveTrade, tickerSymbol, API_BASE }) {
      ...
      const COMPONENT_DEFS = {
        ...
        relativeStrength:   { label: "Strength vs Market",  max: 20, why: "How this stock performs vs SPY." },
        accumulationSignal: { label: "Buy Zone Signal",     max: 15, why: "Has the stock pulled back to a favorable entry?" },
        marketHealth:       { label: "Market Conditions",   max: 10, why: "Is the overall market environment supportive?" },
        trendDurability:    { label: "Trend Durability",    max: 10, why: "How long has the current trend held without breaking?" },
        sectorContext:      { label: "Sector Context",      max: 10, why: "Is this stock's sector in favor right now?" },
      };
            ...
            Object.entries(COMPONENT_DEFS).map(([key, def]) => {
              ...
              return h("div", { key },
                h("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 3 } },
                  ...
                  h("span", { style: { fontFamily: "var(--tt-font-mono)", fontSize: "var(--ds-fs-meta)", color: tone, fontWeight: 700 } },
                    val.toFixed(0), h("span", { style: { color: "var(--ds-text-muted)", fontWeight: 500 } }, `/${def.max}`),
                  ),
                ),
                h("div", { style: { height: 4, background: "rgba(255,255,255,0.06)", borderRadius: 2, overflow: "hidden" } },
                  h("div", { style: { width: `${pct}%`, height: "100%", background: tone, transition: "width 0.3s ease" } }),
                ),
                h("div", { style: { marginTop: 3, fontSize: 10, color: "var(--ds-text-muted)" } }, def.why),
              );
            }),
          ),
        ),
        ...
        (Number.isFinite(rsRank) || rs || sector) && h(Panel, { title: "📈 Strength + Sector Context" },
          h("div", { style: { display: "flex", flexDirection: "column", gap: 8 } },
            Number.isFinite(rsRank) && h("div", null,
              ...
              h("div", { style: { fontFamily: "var(--tt-font-mono)", fontSize: 14, fontWeight: 700, marginTop: 2, color: rsRank >= 70 ? "#34d399" : rsRank >= 50 ? "var(--ds-text-body)" : "#f87171" } },
                rsRank.toFixed(0), h("span", { style: { fontSize: 10, fontWeight: 500, color: "var(--ds-text-muted)" } }, "/100"),
                rsRank >= 80 && h("span", { style: { marginLeft: 8, fontSize: 10, padding: "2px 6px", borderRadius: 999, background: "rgba(52,211,153,0.10)", color: "#34d399", fontWeight: 700, letterSpacing: "0.05em" } }, "TOP 20%"),
              ),
            ),
            rs && h("div", { style: { display: "flex", gap: 12, fontSize: "var(--ds-fs-meta)", color: "var(--ds-text-muted)", flexWrap: "wrap" } },
    ...
    function OptionsTabPanel({ tickerSymbol, API_BASE, mode: modeProp }) {
      ...
      /* 2026-06-01 — Day-trade play surface in the right-rail Options tab.

         Operator: "when the row is clicked it loads the right rail
         options tab but the day trade options plays are not shown
         (Ok if no longer valid but then going back to the issue of
         suppressing)."

         For SPY / QQQ / IWM the right-rail Options tab now ALSO fetches
         /timed/options/all to read the day-trade play for that ticker
         and renders it as a separate panel above the main ladder. If
         the play got suppressed (strike drift / after-close / low vol)
      ...
      /* 2026-06-01 — Day-trade panel render. Shows either the live
         play (with strike + expiration + max-loss + breakeven) or the
         suppression reason. SPY/QQQ/IWM only. */
      const _dayTradePanel = (() => {
        ...
        if (play) {
          ...
          const strikeBlurb = strike
            ? (flavor === "straddle" ? `$${strike} straddle` : `$${strike} ${flavorLabel.toLowerCase()}`)
            : (play.primary?.label || "Day-trade play");
          return h(Panel, {
            title: `DAY TRADE · ${play.day_trade_dte}DTE`,
            color: headerColor,
            action: h("span", {
              style: { display: "inline-flex", alignItems: "center", gap: 4, padding: "2px 8px", borderRadius: 999, background: `${flavorColor}22`, color: flavorColor, fontSize: 10, fontWeight: 700, letterSpacing: "0.04em", border: `1px solid ${flavorColor}55` },
            }, flavorLabel),
          },
            h("div", { style: { fontSize: 14, fontWeight: 700, fontFamily: "var(--tt-font-mono)", color: "var(--ds-text)", marginBottom: 4 } }, strikeBlurb),
            h("div", { style: { fontSize: 11, color: "var(--ds-text-muted)", marginBottom: 8 } },
              `${play.primary?.expiration?.label || `${play.day_trade_dte}DTE`} · spot $${spot ? spot.toFixed(2) : "?"}`),
            h("div", { style: { fontSize: 11, color: "var(--ds-text-body)", lineHeight: 1.55, marginBottom: 8 } },
              play.primary?.rationale || play.primary?.label || ""),
            h("div", { style: { fontSize: 11, fontFamily: "var(--tt-font-mono)", color: "var(--ds-text-muted)" } },
              `Max loss $${play.primary?.max_loss_usd || "?"}`,
              play.primary?.breakeven
                ? ` · BE $${play.primary.breakeven.toFixed(2)}`
                : (play.primary?.breakeven_up ? ` · BE $${play.primary.breakeven_down.toFixed(2)} / $${play.primary.breakeven_up.toFixed(2)}` : ""),
            ),
          );
        }
        ...
        return h(Panel, {
          ...
          h("div", { style: { fontSize: 12, color: "var(--ds-text-muted)", lineHeight: 1.55 } },
            "No day-trade play surfaced for this ticker right now. Reason: ",
      ...
      return h("div", { style: { display: "flex", flexDirection: "column", gap: "var(--ds-space-3)", position: "relative" } },
        ...
        h(Panel, {
          ...
          // know much beyond buying calls and puts, so we want to make
          // sure they can understand how these other strategies may
          // work." Walks through each leg in plain English + the
          // expiry-day P&L scenarios. Generated from primary.legs +
          // primary.strikes + primary.premium so it auto-adapts to
          // every archetype (long call / put / vertical spread / CSP /
          // covered call / straddle / moonshot / LETF / stock).
          (() => {
            ...
            const scenarios = (() => {
              ...
              if (arch.includes("vertical")) {
                ...
                return [
                  `📈 You pay ${fmt$(netPrem)} per spread × ${contracts} = ${fmtU(maxLoss)} upfront (your max loss).`,
                  `🎯 You profit if ${sym} ${dir} past ${fmt$(beSingle)} by ${exp}.`,
                  `🏆 At or beyond ${fmt$(farStrike)} you collect the full ${fmtU(maxGain)} — the spread width (${fmt$(Math.abs((shortStrike || 0) - (longStrike || 0)))}) minus what you paid.`,
                  `🔒 The short leg at ${fmt$(farStrike)} caps your upside — you keep ${fmtU(maxLoss)} of capital at risk in exchange for ${(maxGain / Math.max(1, maxLoss)).toFixed(1)}× the return profile vs a long ${isBull ? "call" : "put"} alone.`,
                  `⛔ Below ${fmt$(closeStrike)} at expiry, both legs expire worthless — you lose the full ${fmtU(maxLoss)}.`,
                ];
              }
              if (arch.includes("long_call") || arch.includes("moonshot_call")) {
                return [
                  `📈 You pay ${fmt$(netPrem)} per contract × ${contracts} = ${fmtU(maxLoss)} upfront (your max loss).`,
                  `🎯 You profit if ${sym} closes above ${fmt$(beSingle)} by ${exp}.`,
                  `🏆 Upside is unlimited — every $1 ${sym} rises past breakeven is roughly +$100 per contract.`,
                  `⛔ Below ${fmt$(longStrike)} at expiry the call expires worthless — you lose the full ${fmtU(maxLoss)}.`,
                ];
              }
              if (arch.includes("long_put") || arch.includes("moonshot_put")) {
                return [
                  `📉 You pay ${fmt$(netPrem)} per contract × ${contracts} = ${fmtU(maxLoss)} upfront (your max loss).`,
                  `🎯 You profit if ${sym} closes below ${fmt$(beSingle)} by ${exp}.`,
                  `🏆 Max gain ≈ ${fmtU((longStrike - netPrem) * 100 * contracts)} if ${sym} → $0 by expiry.`,
                  `⛔ Above ${fmt$(longStrike)} at expiry the put expires worthless — you lose the full ${fmtU(maxLoss)}.`,
                ];
              }
              if (arch.includes("cash_secured_put") || arch.includes("csp")) {
                return [
                  `💰 You COLLECT ${fmt$(netPrem)} per contract × ${contracts} = ${fmtU(Number.isFinite(maxGain) ? maxGain : netPrem * 100 * contracts)} premium upfront.`,
                  `🏦 You must set aside ${fmtU((longStrike || 0) * 100 * contracts)} cash as collateral (the broker holds it while the put is open).`,
                  `🎯 If ${sym} stays above ${fmt$(longStrike)} at ${exp}, the put expires worthless — you keep the full premium.`,
                  `📦 If ${sym} drops below ${fmt$(longStrike)}, you're ASSIGNED 100 shares at $${longStrike} per contract. Effective cost basis: ${fmt$((longStrike || 0) - (netPrem || 0))} (strike minus collected premium).`,
                ];
              }
              if (arch.includes("covered_call")) {
                return [
                  `📦 You must already own 100 shares of ${sym} per contract you sell.`,
                  `💰 You COLLECT ${fmt$(netPrem)} per contract × ${contracts} = ${fmtU(Number.isFinite(netPrem) ? netPrem * 100 * contracts : 0)} premium upfront.`,
                  `🎯 If ${sym} stays below ${fmt$(longStrike)} at ${exp}, the call expires worthless — you keep the premium AND your shares.`,
                  `🚪 If ${sym} closes above ${fmt$(longStrike)}, your shares are CALLED AWAY at $${longStrike}. You give up the upside above that strike in exchange for the premium.`,
                ];
              }
              if (arch.includes("straddle")) {
                return [
                  `📈📉 You BUY both a call AND a put at the same strike (${fmt$(longStrike)}). Total cost: ${fmt$(netPrem)} per straddle × ${contracts} = ${fmtU(maxLoss)} (your max loss).`,
                  `🎯 You profit on a BIG move EITHER direction — past ${fmt$(beUp)} (up) or below ${fmt$(beDn)} (down) by ${exp}.`,
                  `⚠ You LOSE money if ${sym} stays near ${fmt$(longStrike)} — both legs decay every day.`,
                  `🏆 Best for binary events: earnings, FOMC, drug data, tariff announcements where the move is BIG but direction is unclear.`,
                ];
              }
              if (arch.includes("leveraged_etf") || arch.includes("letf")) {
                return [
                  `📈 BUY ${primary.contracts || "?"} shares of ${primary.letf_ticker || "the leveraged ETF"} — a ${primary.factor || 3}× daily-reset ETF that tracks ${sym}.`,
                  `⚡ A 1% move in ${sym} ≈ a ${primary.factor || 3}% move in the LETF (intraday only — overnight gaps and daily compounding can drift the relationship).`,
                  `⏳ Hold ≤ 5 trading days. LETFs decay vs the underlying over longer windows due to daily-reset path dependence ("volatility drag").`,
                  `🛒 Standard share order — no expiration, no Greeks, no premium decay. Just leveraged exposure.`,
                ];
              }
              return null;
            })();
            ...
            return h("div", {
              ...
              scenarios && h("div", null,
                h("div", { style: { fontSize: 9, color: "var(--ds-text-faint)", letterSpacing: "0.05em", marginBottom: 3 } }, `WHAT HAPPENS AT ${String(exp).toUpperCase()}`),
                scenarios.map((s, i) => h("div", {
                  key: i,
                  style: { fontSize: 11, color: "var(--ds-text-body)", lineHeight: 1.5, marginBottom: 3 },
                }, s)),
              ),
            );
          })(),
    ...
    return function TickerDetailRightRail({
        ...
        //   Snapshot / Trader / Setup → trader plan (SL + TPs + Entry)
        //   Investor                  → investor plan (avg cost, accum zone, weekly support/resistance)
        //   Options                   → options strikes + breakeven (when held / suggested)
        //   Catalysts / Technicals / Fundamentals / Chart → no overlay
        const investorPriceLines = useMemo(() => {
          ...
          // Cost basis = green solid horizontal anchor.
          if (pos?.avg_entry > 0) {
            out.push({
          ...
          // Accumulation zone — dashed cyan around the buy-zone band.
          if (az?.inZone && Number.isFinite(Number(az.zoneTop)) && Number.isFinite(Number(az.zoneBottom))) {
            out.push({
              price: Number(az.zoneTop),
              color: "rgba(34,211,238,0.6)", lineWidth: 1, lineStyle: 2,
              axisLabelVisible: true, title: `Buy Zone High $${Number(az.zoneTop).toFixed(2)}`,
            });
            out.push({
              price: Number(az.zoneBottom),
              color: "rgba(34,211,238,0.6)", lineWidth: 1, lineStyle: 2,
              axisLabelVisible: true, title: `Buy Zone Low $${Number(az.zoneBottom).toFixed(2)}`,
            });
          } else if (Number.isFinite(Number(az?.entryPrice))) {
            out.push({
              price: Number(az.entryPrice),
              color: "rgba(34,211,238,0.6)", lineWidth: 1, lineStyle: 2,
              axisLabelVisible: true, title: `Buy Zone $${Number(az.entryPrice).toFixed(2)}`,
            });
          }
        ...
        // 2026-05-30 — Fetch options/ticker on BOTH OPTIONS and SETUP
        // (Trader) tabs because the Trader tab now displays a confluence
        // verdict chip inherited from the same engine.
        ...
        const optionsPriceLines = useMemo(() => {
          ...
          if (Number.isFinite(Number(strike)) && strike > 0) {
            out.push({
              ...
              axisLabelVisible: true, title: `Long Strike $${Number(strike).toFixed(2)}`,
            });
          }
          ...
          if (Number.isFinite(Number(be))) {
            out.push({
              price: Number(be), color: "rgba(96,165,250,0.85)", lineWidth: 1, lineStyle: 2,
              axisLabelVisible: true, title: `Breakeven $${Number(be).toFixed(2)}`,
            });
          }
          if (Number.isFinite(Number(beUp))) {
            out.push({
              price: Number(beUp), color: "rgba(96,165,250,0.85)", lineWidth: 1, lineStyle: 2,
              axisLabelVisible: true, title: `BE Up $${Number(beUp).toFixed(2)}`,
            });
          }
          if (Number.isFinite(Number(beDn))) {
            out.push({
              price: Number(beDn), color: "rgba(96,165,250,0.85)", lineWidth: 1, lineStyle: 2,
              axisLabelVisible: true, title: `BE Dn $${Number(beDn).toFixed(2)}`,
            });
          }
          // Multi-bagger targets (Moonshot only).
          const mbt = p.multi_bagger_targets;
          if (mbt) {
            const a = Number(mbt["2x_underlying_at"]);
            const b = Number(mbt["3x_underlying_at"]);
            if (Number.isFinite(a)) {
              out.push({ price: a, color: "rgba(34,197,94,0.55)", lineWidth: 1, lineStyle: 2, axisLabelVisible: true, title: `🚀 2× @ $${a.toFixed(2)}` });
            }
            if (Number.isFinite(b)) {
              out.push({ price: b, color: "rgba(34,197,94,0.75)", lineWidth: 1, lineStyle: 2, axisLabelVisible: true, title: `🚀 3× @ $${b.toFixed(2)}` });
            }
          }
          return out.length > 0 ? out : EMPTY_PRICE_LINES;
        }, [optionsTabData]);
                ...
                {autopsyModalLoading ? (
                  ...
                  <div className="p-3 md:p-4 flex-1 min-h-0 overflow-y-auto flex flex-col gap-3">
                    ...
                    {/* 2026-05-27 (PR #323 + PR #324) — Receipt-style chronological event log.
                        ...
                        Builds rows from:
                          - ENTRY: synthesized from trade.entry_ts + .shares + .entry_price
                          - TRIMs/EXIT: from trade.history[] (already populated by the
                            engine on each trim/close — see worker/index.js line ~17620
                            for the event shape).

                        Renders a compact table with running share count + cumulative
                        realized P&L so it reads like a fill receipt. Total row at the
                        bottom sums everything. */}
                    {(() => {
                      const events = [];
                      ...
                      // 4) Build display rows with running share count + cumulative P&L.
                      let runningShares = 0;
                      let cumPnl = 0;
                      ...
                      // Total realized vs current open (mark-to-market). For
                            ...
                            <table className="w-full text-[12px] tabular-nums" style={{ fontVariantNumeric: "tabular-nums", borderCollapse: "collapse", minWidth: 560 }}>
                              <thead>
                                <tr className="text-[10px] uppercase tracking-wider text-[#6b7280]">
                                  ...
                                  <th className="text-right px-3 py-1.5 font-medium">Held after</th>
                                </tr>
                              </thead>
                              <tbody>
                                {displayRows.map((r, i) => {
        ...
        /* ═══════════════════════════════════════════════════════════════
         * V2 RIGHT RAIL (2026-05-01)
         *
         * Restructured per tasks/right-rail-v2-spec-2026-05-01.md:
         * - 50 sections collapsed to ~16 panels across 4 tabs
         * - All panels use ds-glass surface + ds-metric / ds-card primitives
         * - Removed 3 dead `{false && ...}` blocks
         * - Eliminated duplicate Investor/Analysis Model Guidance
         * - Cohesive with the rest of the v2 surfaces (gold accent, JBM nums)
         *
         * Toggle: window._dsV2RailEnabled = false to fall back to legacy
         * ════════════════════════════════════════════════════════════════ */
        ...
        if (v2Enabled && !modalOnly) {
          ...
          // 2026-05-29 — added INVESTOR. Was previously absent from this
          // whitelist so tapping the Investor tab silently fell back to
          // SNAPSHOT (the body content of the existing INVESTOR branch
              ...
              >
                    ...
                    <div className="flex items-center gap-2 min-w-0" style={{ flexWrap: "wrap" }}>
                      <div className="ds-tickercard__logo" style={{ width: 28, height: 28 }} ref={(el) => {
                        if (el && !el.dataset.dsInit && window.DS) {
                          el.dataset.dsInit = "1";
                          try { el.replaceWith(window.DS.tickerLogo(tickerSymbol, { size: 28 })); } catch (_) {}
                        }
                ...
                >
                      ...
                      {(ticker?.entry_path || ticker?.setup_name) && (
                        <Panel title="Setup">
                          <div style={{ display: "flex", flexDirection: "column", gap: "var(--ds-space-1)" }}>
                            <div style={{ fontSize: "var(--ds-fs-emph)", color: "var(--ds-text-display)", fontWeight: 600 }}>
                              {ticker?.setup_name || ticker?.entry_path || "—"}
                            </div>
                            {ticker?.setup_grade && <span className="ds-chip ds-chip--sm ds-chip--accent" style={{ alignSelf: "flex-start" }}>{ticker.setup_grade}</span>}
                          </div>
                        </Panel>
                      )}
                  ...
                  {v2RailTab === "FUNDAMENTALS" && (() => {
                    ...
                    const resultChip = (r) => {
                      // Compact labels for the Earnings History grid — the
                      // table is shown in the right rail which on mobile is
                      // ~390px wide. "Beat & Raise" (12 chars) was the
                      // widest chip and pushed the Result column past the
                      // grid edge, causing horizontal overflow / clipping.
                      // Short labels keep the grid inside the rail.
                      // `title` preserves the full label on hover.
                      switch (r) {
                        case "beat_raise": return { label: "B+R",    title: "Beat & Raise", cls: "ds-chip--up" };
                        case "beat":       return { label: "Beat",   title: "Beat",         cls: "ds-chip--up" };
                        case "inline":     return { label: "In-line", title: "In-line",     cls: "ds-chip--solid" };
                        case "miss":       return { label: "Miss",   title: "Miss",         cls: "ds-chip--dn" };
                        default:           return null;
                      }
                    };
                        ...
                        {Array.isArray(earn.history) && earn.history.length > 0 && (
                          <Panel title="Earnings History" action={<span className="ds-chip ds-chip--sm">{earn.history.length} qtr{earn.history.length === 1 ? "" : "s"}</span>}>
                            {/* Bug 2026-05-20 (user screenshot): the grid
                                ...
                                rail on mobile (~358px usable). Wrap in
                                overflow-x:auto as a safety net AND tighten
                                the column widths. Combined with the
                                shortened resultChip labels above (B+R), the
                                table now fits without scrolling on phone
                                viewports while still being scrollable if
                                the rail is somehow narrower. */}
                            <div style={{ overflowX: "auto", marginInline: "calc(-1 * var(--ds-space-2))", paddingInline: "var(--ds-space-2)" }}>
                              <div style={{ display: "grid", gridTemplateColumns: "minmax(62px, 1fr) 52px 52px 56px 56px 60px", gap: "var(--ds-space-1)", fontFamily: "var(--tt-font-mono)", fontSize: "var(--ds-fs-meta)", minWidth: 0 }}>
                                {sortHeader("date", "Date", "left")}
                                {sortHeader("eps_actual", "EPS")}
                                  ...
                                  return (
                                    <React.Fragment key={`er-${i}`}>
                                      ...
                                      <div style={{ textAlign: "right" }}>
                                        {rChip ? <span className={`ds-chip ds-chip--sm ${rChip.cls}`} style={{ fontSize: 9 }} title={rChip.title}>{rChip.label}</span> : <span style={{ color: "var(--ds-text-faint)" }}>—</span>}
                                      </div>
                                    </React.Fragment>
                                  );
                                })}
        ...
        return (
          ...
          <div className="w-full h-full flex flex-col" style={modalOnly ? { position: "absolute", width: 1, height: 1, overflow: "hidden", opacity: 0, pointerEvents: "none" } : undefined}>
                ...
                <div className="p-6 pt-4">
                  {!window._ttIsPro && railTab !== "ANALYSIS" && railTab !== "INVESTOR" && railTab !== "TECHNICALS" ? (
                    ...
                    (() => {
                      ...
                      // Mode so the language and guidance are aligned."
                      // Translates the investor_stage into an explicit
                      // action with position context. Renders at the top
                      // of the Investor tab as the headline takeaway.
                      const LANE_ACTION = {
                        accumulate: {
                          label: "ACCUMULATE",
                          color: "#34d399", bg: "rgba(52,211,153,0.10)", border: "#00c853",
                        ...
                        core_hold: {
                          label: "CORE HOLD",
                          color: "#60a5fa", bg: "rgba(96,165,250,0.10)", border: "#3b82f6",
                        ...
                        watch: {
                          label: "WATCH",
                          color: "#fbbf24", bg: "rgba(251,191,36,0.10)", border: "#f59e0b",
                        ...
                        reduce: {
                          label: "REDUCE",
                          color: "#f87171", bg: "rgba(248,113,113,0.10)", border: "#ef4444",
                        ...
                        research_on_watch: {
                          ...
                          color: "#a78bfa", bg: "rgba(167,139,250,0.10)", border: "#8b5cf6",
                        ...
                        research_low: {
                          ...
                          color: "#a78bfa", bg: "rgba(167,139,250,0.08)", border: "#7c3aed",
                          action: "Pass",
                      ...
                      return (
                        <div className="space-y-5">
                          {/* Header — score on right, ticker on left */}
                          <div>
                            <div className="flex items-center justify-between mb-1">
                              <div>
                          ...
                          <div className="rounded-xl p-3.5 border" style={{ background: lane.bg, borderColor: lane.border + "55" }}>
                            <div className="flex items-center justify-between mb-1.5 gap-3">
                              <span
                                className="text-[10px] font-bold tracking-[0.14em] px-2 py-0.5 rounded"
                                style={{ color: lane.color, background: lane.bg, border: `1px solid ${lane.color}55` }}
                              >
                                {lane.label}
                              </span>
                            ...
                            {/* Buy-zone hint — visible when accumZone.inZone */}
                            {d.accumZone?.inZone && (
                              <div className="mt-3 pt-3 border-t" style={{ borderColor: lane.color + "30" }}>
                                ...
                                {Array.isArray(d.accumZone.signals) && d.accumZone.signals.length > 0 && (
                                  <div className="flex flex-wrap gap-1.5 mt-1.5">
                                    {d.accumZone.signals.map((s, i) => (
                                      <span key={i} className="text-[9px] px-1.5 py-0.5 rounded bg-[#00c853]/10 text-[#34d399]/90 border border-[#00c853]/20">
                                        {SIGNAL_LABELS[s] || String(s).replace(/_/g, " ")}
                                      </span>
                                    ))}
                                  </div>
                                )}
            ...
            {chartExpanded && (() => {
              ...
              // V15 P0.7.159 — Portal the fullscreen chart modal to document.body.
              // The right-rail wrapper (.tt-rail-mobile) has overflow-y:auto +
              // position:fixed + z-index:60 which creates a stacking context.
              // Any position:fixed child would be trapped inside that context
              // and could render behind other elements or clip on some devices.
              // Portalling to document.body guarantees viewport-level positioning
              // and correct z-index ordering regardless of parent stacking contexts.
              ...
              return _usePortal
                ? window.ReactDOM.createPortal(_modalJsx, _portalTarget)
                : _modalJsx;
            })()}
