...
const SVGBubble = memo(({
  ...
  useEffect(() => {
    ...
    if (!a.init) {
      ...
      a.cy = targetY;
      a.init = true;
      if (gRef.current) gRef.current.setAttribute("transform", `translate(${targetX.toFixed(1)}, ${targetY.toFixed(1)})`);
      return;
    }
    const sx = a.cx,
      sy = a.cy;
...
function TimeTravelSlider({
  ...
  const minTimestamp = availableTimestamps.length > 0 ? availableTimestamps[0] : roundToStep(Date.now() - 7 * 24 * 60 * 60 * 1000);
      ...
      try {
        ...
        for (let i = 0; i < tickerSymbolsArray.length; i += MAX_CONCURRENT) {
          ...
          const batchResults = await Promise.all(batch.map(async ticker => {
            try {
              const res = await fetch(`/timed/trail?ticker=${encodeURIComponent(ticker)}&include_kanban=1`);
              if (res.ok) {
                const json = await res.json();
                if (json.ok && json.trail && Array.isArray(json.trail)) {
                  return {
                    ticker,
                    trail: json.trail
                  };
  ...
  if (compact) {
    ...
    }, "Cancel")), isActive && !loadingTrails && React.createElement(React.Fragment, null, React.createElement("button", {
      onClick: () => {
        if (!isPlaying) {
...
function OpportunitiesPanel({
  ...
  return React.createElement("div", {
  ...
  }, "Prime ", counts.prime, " \u2022 Eligible ", counts.eligibleOnly, " \u2022 Watch", " ", counts.watch, " \u2022 Total ", counts.all, React.createElement("span", {
    className: "ml-2 text-[#4b5563]"
  }, "\u2022 \uD83D\uDFE2 ", counts.active, " \u26D4 ", counts.invalidated, " \u2705", " ", counts.completed))), React.createElement("div", {
    className: "flex items-center gap-2"
    ...
    value: "RANK"
  }, "Sort: Rank"), React.createElement("option", {
    value: "ABC"
  }, "Sort: ABC"), React.createElement("option", {
    value: "DCHG"
  }, "Sort: Daily %")), view === "today" && React.createElement("button", {
  ...
  return React.createElement(React.Fragment, null, showWelcomeDashboard && React.createElement(DashboardWelcomeModal, {
    ...
    const chipColor = e => {
      if (e.epsActual != null && e.epsEstimate != null) {
        return e.epsActual >= e.epsEstimate ? "border-green-500/30 bg-green-500/10 text-green-300" : "border-rose-500/30 bg-rose-500/10 text-rose-300";
      }
    ...
    return React.createElement("div", {
    ...
    }, earningsEvents.length, " earnings")), React.createElement("div", {
      ...
      }, formatDate(e.date)), formatHour(e.hour) && React.createElement("span", {
    ...
    const renderPulseChip = sym => {
      ...
      const chg = (() => {
        if (!td) return null;
        const dc = getDailyChange(td);
        return dc && Number.isFinite(dc.dayChg) ? dc.dayChg : null;
      })();
      return React.createElement("button", {
        key: `mp2-${sym}`,
        onClick: () => handleTickerSelect(sym),
        className: "flex items-center gap-1.5 px-2 py-1 rounded-lg hover:brightness-125 flex-shrink-0 transition-all",
        style: {
          background: pillBg,
          border: borderStyle
        }
      ...
      }, pct >= 0 ? "+" : "", pct.toFixed(2), "%"), chg !== null && React.createElement("span", {
        className: `text-[8px] tabular-nums ${up ? "text-[#00e676]/70" : "text-rose-400/70"}`
      }, chg >= 0 ? "+" : "", chg.toFixed(2)));
    };
    ...
    return React.createElement("div", {
      ...
      className: "text-[9px] text-[#4b5563] tabular-nums"
    }, "updated ", freshness)), row1Chips.length > 0 && React.createElement("div", {
      className: "flex items-center gap-2 overflow-x-auto pb-1",
      style: {
        scrollbarWidth: "none"
      }
    }, row1Chips), row2Chips.length > 0 && React.createElement("div", {
      className: "flex items-center gap-2 overflow-x-auto pt-1 pb-1",
      style: {
        scrollbarWidth: "none"
      }
    }, row2Chips));
    ...
    const chip = (t, isGain, pctKey = "_pct", priceKey = "_price") => {
      ...
      return React.createElement("button", {
        ...
        onClick: () => handleTickerSelect(sym),
        title: title,
        className: `px-2 py-0.5 rounded-lg text-[10px] font-semibold flex items-center gap-1 hover:brightness-125 transition-all ${textCls}`,
        style: {
          background: chipBg,
          border: borderStyle
        }
    ...
    const moversRow = (label, badgeCls, gainers, losers, pctKey, priceKey) => React.createElement("div", {
      ...
      className: "text-[#6b7280] font-semibold whitespace-nowrap"
    }, "Gainers"), gainers.map(t => chip(t, true, pctKey, priceKey))), React.createElement("div", {
      className: "w-px h-4 bg-white/10"
    }), React.createElement("div", {
      className: "flex items-center gap-1.5"
    }, React.createElement("span", {
      className: "text-[#6b7280] font-semibold whitespace-nowrap"
    }, "Losers"), losers.map(t => chip(t, false, pctKey, priceKey))));
    ...
    }, (rthGainers.length > 0 || rthLosers.length > 0) && moversRow("RTH", "bg-cyan-500/15 text-cyan-400 border-cyan-500/30", rthGainers, rthLosers, "_pct", "_price"), hasEth && moversRow("EXT", "bg-amber-500/15 text-amber-400 border-amber-500/30", ethGainers, ethLosers, "_ethPct", "_ethPrice"));
    ...
    className: "text-[10px] uppercase tracking-[0.18em] text-[#4b5563] mb-2"
  }, "Search And Curate Your View"), React.createElement("div", {
    ...
    d: "M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z",
    clipRule: "evenodd"
  })), React.createElement("span", null, "\u23F1\uFE0F Time Travel"), React.createElement("span", {
