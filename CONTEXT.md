...
- **Isolating a partner's bookkeeping is not the same as ignoring it (2026-09-24):** `/bridge/options/order` places the operator's order and every partner mirror in one call, returning the operator's result at the top level with `fanout: {accounts, results}` beside it — isolation that is deliberate and documented, so a partner can never move the operator's day-loss ledger. `worker/options-auto-mirror.js` then never read `fanout` at all, so a partner leg that failed while the operator's filled was neither recorded nor said out loud. The near-miss is the instructive part: `bridgeResponseIsOk` HAS a fan-out clause written for exactly this hazard ("fan-out wrappers always used to return `{ok:true}` even when every account rejected"), and it tests `parsed.fanout === true` — which matches the EQUITY route's flat `{fanout:true, results:[…]}` and not the options route's nested object under the same key. The guard looked present in review and was dead in production; scope a guard to the shape, not the name. Partner legs are now graded through `deriveMirrorDecision` (the operator's own grader — a second grader is a second definition of "mirrored") and a partner reduce the broker did not take pages per signal/event/account. **No per-partner reconciler was added, on purpose:** a reconciler-driven close goes back through the options webhook, which fans out again, and `clampReduceToHeld` clamps each account to what it actually holds, so an account already flat no-ops. Check whether the existing heal already generalises before building a second one.
