...
const CompactCard = React.memo(function CompactCard({
  ...
  const _slSaneForDir = raw => {
    if (!Number.isFinite(raw) || raw <= 0 || !Number.isFinite(price) || price <= 0) return false;
    if (dir === "LONG" && raw >= price) return false;
    if (dir === "SHORT" && raw <= price) return false;
    return true;
  };
  const _engineSL = Number(t?.sl ?? t?.sl_price ?? t?.stop_loss ?? t?.stop);
  const _posSL = hasPos ? Number(t?.position_sl) : NaN;
  const slRaw = (() => {
    const engineOk = _slSaneForDir(_engineSL);
    const posOk = hasPos && _slSaneForDir(_posSL);
    if (engineOk) return _engineSL;
    if (posOk) return _posSL;
    if (Number.isFinite(_engineSL) && _engineSL > 0) return _engineSL;
    if (Number.isFinite(_posSL) && _posSL > 0) return _posSL;
    return NaN;
  })();
  ...
  const slSane = _slSaneForDir(slRaw) ? slRaw : null;
  const tpSane = (() => {
...
function SimpleKanbanTable({
  ...
  const MiniProgressBar = ({
    ...
    const price = Number(t?.price ?? t?.close);
    const hasPos = !!t?.has_open_position;
    const _slSaneForDir2 = raw => {
      if (!Number.isFinite(raw) || raw <= 0 || !Number.isFinite(price) || price <= 0) return false;
      if (effectiveDir === "LONG" && raw >= price) return false;
      if (effectiveDir === "SHORT" && raw <= price) return false;
      return true;
    };
    const _engineSL2 = Number(t?.sl ?? t?.sl_price ?? t?.stop_loss ?? t?.stop);
    const _posSL2 = hasPos ? Number(t?.position_sl) : NaN;
    const slRaw = (() => {
      const engineOk = _slSaneForDir2(_engineSL2);
      const posOk = hasPos && _slSaneForDir2(_posSL2);
      if (engineOk) return _engineSL2;
      if (posOk) return _posSL2;
      if (Number.isFinite(_engineSL2) && _engineSL2 > 0) return _engineSL2;
      if (Number.isFinite(_posSL2) && _posSL2 > 0) return _posSL2;
      return NaN;
    })();
    const tpRaw = numFromAny(t?.tp ?? t?.tp_trim);
    const slSane = _slSaneForDir2(slRaw) ? slRaw : null;
