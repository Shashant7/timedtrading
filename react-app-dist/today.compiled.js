...
function cloudDeskPlanCopy(w) {
  ...
  const playWord = role === "fire" ? "Cloud Pivot fire, paper size" : role === "leader" ? "leader curl" : role === "follow" ? `follow ${leader || "leader"}` : role === "catalyst" ? "catalyst if/then" : role === "stalk" ? "magnet stalk" : "desk watch";
...
function SetupFamiliesStrip({
  ...
  }, "Admitted runners — Confirm-stack, Cloud Pivot, Continuation. Cloud Desk above is the weekend/night watch. Size stays paper until attribution holds."));
  if (!window._ttIsPro) {
  ...
  const deskStrip = deskWatch.length ? h("div", {
    ...
    className: "tt-ready__sub"
  }, "Inspect the 10m 5/12 tape, 1H magnets, day2 holds, and BTC/ETH/SPY/QQQ leaders. Not a sized ticket — paper until a Families card admits the name."), h("ul", {
    className: "tt-desk-howto"
  }, h("li", null, "FIRE is a 10m 5/12 print, not a sized entry. Paper 0.1× until a Families card below admits the name."), h("li", null, "Leaders first: BTCUSD, ETHUSD, SPY, QQQ. A follower only matters on the same side as the leader curl."), h("li", null, "Magnet is the cover. Ride toward the arrow; flatten when tagged. If/then chips are gates (long over X / short under Y), not market orders.")), h("div", {
