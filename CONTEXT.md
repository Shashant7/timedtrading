...
- **`tf_tech.W.atr.xs` is flip-only AND sign-mirrored** — `indicators.js` emits `atrCross` only when `b.stFlip`, and sets `xs = stDir < 0 ? -1 : 1`, so `xs === 1` means **bearish** under the Pine convention (`-1 = bull`) used everywhere else in `investor.js`. Both consumers (`criteria.weeklyST`, `components.trendDurability`) read it as bullish, so `trendDurability` scored 0 on all 35 recorded entries and Weekly SuperTrend never became an invalidation candidate (0 of 28 exits). Read the persistent `stDir` instead — the daily equivalent (`dStDir === -1`) is already correct. Flag: `deep_audit_investor_weekly_st_dir_fix`.
