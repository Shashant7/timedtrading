...
peers skew (npm/cli#9787). Not a Kanban or dist-rebuild bug.

**Fix:** Pin `vitest` to `4.1.11`, add `react`/`react-dom` 18.3.1 peers,
and set `.npmrc` `legacy-peer-deps=true` (landed on main via #1406).
...
- **Keepalive false page on overnight lightweight + hard-fail unauthorized (2026-08-08):** Overnight `feed_mode=lightweight` ticks */5 so age ~270s is healthy, but keepalive used a 180s threshold, called `/feed/run-once`, got `unauthorized` (repo `TIMED_API_KEY` ≠ tt-feed worker secret), and `jq -e '.ok == true'` failed the job → email. Fix: lightweight stale gate 600s; soft-fail heal errors (exit 0 — watchdog pages); heartbeat/scoring heals only when monolith `operatingHours=true`. Sync GitHub secret to tt-feed's `TIMED_API_KEY` or kicks stay no-ops.
...
- **Investor trim email ≠ broker fill — resync held names; bypass flat ones (2026-08-06):** Aug 5 MFE Extension trims booked on the model/email but bridge rejected all six. Resync only tickers with live Roth shares (PLTR=2, NVDA=1): OPEN + unsuppressed + `broker_remaining_qty = model_intended_qty = broker qty`. Names with **zero** broker shares (CRS/WTS/IWM/GE) must be **bypassed** — `CLOSED` + `mirror_suppressed=bypass_no_broker_position` — do not leave them OPEN at rem=0 (that invites failed trim retries). Release competing CLOSED orphans' qty claims. See `skills/broker-bridge.md` → "Accept broker qty as post-trim baseline".
...
## 2026-09-05 — The ratchet was firing; the execution layer re-gated it as a soft exit
- **Symptom:** 17 of 21 real core winners (peak >= 1.5%) closed under 40% of peak while the "unsuppressible" MFE ratchet was live. FLR sat under its floor for six hours of RTH and died at `max_loss`. TSLA's runner: ratchet said exit at +8.38% (floor +8.47%) twelve times; the trade closed `sl_breached` at +2.73% blended.
- **Cause:** `classifyKanbanStage` returned "exit" with `mfe_ratchet_giveback`, and the lifecycle exit block treated every non-SL reason as a soft signal exit: trimmed-runner pullback shield (`[EXIT SHIELD] TSLA runner EXIT blocked: c7289_15,c3450_15,c7289_30 reason=mfe_ratchet_giveback`), 30-minute management cadence (the three fires that closed landed a full bucket below their floors), min-age gate, bleeder shield, CIO HOLD.
- **Rule:** "Unsuppressible" has to be true at the layer that calls `closeTradeAtPrice`, not only where the reason is set. Any new hard-class exit reason needs a decision in the lifecycle gate block (`isSLExit` / `_exitIsHardClass` / `_exitIsHard` / the shield) or it is soft by default. The profit lock (`isMfeRatchetExit`) now bypasses those gates, stays RTH-only, and keeps the stale-tick guard. Reading `[MFE_RATCHET]` lines alone proves nothing; read the next line for the same ticker.
- **Also:** Workers Logs are queryable from the shell (`POST /accounts/{id}/workers/observability/telemetry/query`, `$metadata.message INCLUDES`). Three days of `[EXIT SHIELD]` lines answered what a static read of a 100k-line file could not.

## 2026-09-05 — The weekly review runs itself (Friday 17:00 ET)
- **Symptom:** "Read the report cards weekly" was a note in a plan, i.e. a task nobody owns.
- **Rule:** A recurring review is a cron, an email and a page, or it does not exist. `worker/execution-review.js` grades the ledger every Friday 17:00 ET (hourly slot gated by ET day+hour; DST-safe), judges the plan's pass condition, stores KV + 12-week history, emails the operator, posts one Discord line; `/execution-review.html` renders it and can run it on demand. The member Weekly Retrospective moved to the same slot. Grade the week when the week closes, not on Sunday.
...
## 2026-09-07 — A "delete it and let the next tick rewrite it" heal needs a next tick
- **Symptom:** Labor Day, 15:43Z: watchdog red with `301 universe orphans (incomplete onboard: SKHY,SPCX,CBRS,...)`; the 12:59Z run had 0. Every orphan was `hasScore: false`; only 29 `timed:latest:*` keys were left in KV.
- **Cause:** The 9 AM ET SECTOR_MAP completeness monitor treats a `timed:latest` stub older than 24h as stale and DELETES it "so the next live scoring cron starts from a clean slate". On a market holiday there is no next scoring cron. (The 60m candle check next to it had a Monday-only 72h special case; the stub check had none, and neither knew about holidays. On ordinary Mondays the wipe happened too, silently, and 9:30 scoring repopulated from `existing = null`.)
- **Rule:** Staleness of anything the market writes is measured in sessions, not hours. `sessionAwareStaleMs(nowMs)` in `worker/foundation/trading-calendar.js` (24h floor, stretched back to the open of the latest settled RTH session; weekend + holiday tables) now drives both thresholds. Before adding an "older than N hours" rule to a monitor, ask what the rule does on Labor Day at 3 PM. A destructive heal (delete, reset, requeue) must only run when the thing that repairs it is guaranteed to run afterwards.
- **Recovery:** D1 `ticker_latest.payload_json` still had every payload (mixed freshness; D1 is written on change, not per tick); 300 stubs re-seeded via the KV bulk API, `POST /timed/admin/onboard-gaps?limit=3` recomputed the orphan snapshot. Deploying a worker kills an in-flight `waitUntil` heal job and leaves `timed:onboard-gaps:running` set for its 2h TTL; clear the flag before re-triggering. The D1 payloads carry their own old `ts` (Aug 27 for most names), so the 3 PM ET sweep, correctly, judged 292 of the restored stubs stale again and deleted them (second red run, 19:49Z). A restore only holds when a scoring session follows before the next sweep: re-seeded at 19:52Z, scoring resumes 04:00 ET Tuesday (operating hours), the 09:00 sweep then sees fresh `ts`. Do not bump `ts` to make a restore "stick"; that lies to every freshness consumer.

## 2026-09-08 — Webull stops fractional equity at 16:00 ET, not 19:00
...
`tasks/2026-09-09-rank-driver-evaluation.md` and `skills/rank-driver-audit.md`.

**Do not reintroduce (fixed in PR #1442):**
- Missing/null completion or phase fraction treated as “early” (+15 / +3).
- Opposing HTF/LTF strength or incompatible state earning support points.
- Duplicate trigger paths (summary + direct) for the same EMA/dip/squeeze event.
- EXTREME phase interpreted as a favorable “zone change.”
- Fixed sector prior from historical UP rates applied to both sides.
- TD sequential / HMM multiplier keyed to HTF bias instead of candidate side.
- Adaptive rank weight overrides as strings concatenating into the score.
- Legacy `computeDynamicScore` second-layer corridor / squeeze-in-corridor /
  hold-intent / phase-zone-change bonuses on top of `computeRank`.

**Offline only:** `outcome-rank-v2-setup` failed chronological eval — do not
wire into `worker/index.js`. Reproduce: `scripts/evaluate-outcome-rank.mjs`.
