...
poll_backfill_done() {
  ...
  for _ in $(seq 1 90); do
    sleep 3
    local phase
    phase=$(curl -s "${BASE}/timed/admin/backfill-status" -H "X-TT-Admin-Key: ${KEY}" | python3 -c "import json,sys; d=json.load(sys.stdin); print((d.get('status') or {}).get('phase') or '')" 2>/dev/null || echo "")
...
score_ticker() {
  ...
  curl -s -X POST "${BASE}/timed/admin/onboard?ticker=${ticker}&skipBackfill=1" \
    -H "X-TT-Admin-Key: ${KEY}" | python3 -c "import json,sys; d=json.load(sys.stdin); r=(d.get('results') or [{}])[0]; print('ok' if d.get('ok') and (r.get('ok', d.get('ok'))) else (r.get('error') or d.get('error') or 'fail'))"
}
...
for ticker in "${TICKERS[@]}"; do
  ...
  q=$(curl -s "${BASE}/timed/admin/ingestion-status" -H "X-TT-Admin-Key: ${KEY}" | python3 -c "
import json,sys
