...
- **tt-feed owns */1 price cron after cutover**: monolith `wrangler deploy` does not refresh feed heals when `PRICE_FEED_EXTERNAL=true`. Mid-RTH `price_value_freshness` with ~30m cohort ages + live majors → redeploy `worker-feed/`. (2026-07-16: stale tt-feed since Jul 10; redeploy cleared 47 → 0.)
