...
/**
 * Cron handler. Runs sweep → persist → self-heal → incident sync → Discord.
 *
 * Order matters: runSelfHealing reads `sanity_sweep:latest` (or an in-memory
 * sweep), so persist must come first. Heal runs before Discord so the alert
 * can report what was auto-fixed. Fast sweeps still skip Discord to avoid
 * double-paging with the hourly full sweep.
 */
export async function sanitySweepCron(env, ctx, kind = "full") {
  try {
    ...
    if ((failing.length > 0 || warning.length > 0)
        && String(env?.COO_SELF_HEAL || "false").toLowerCase() === "true") {
      try {
        const { runSelfHealing } = await import("./coo/coo-orchestrator.js");
        // Pass the just-run sweep so heal doesn't race a stale KV read.
        healResult = await runSelfHealing(env, { sweep }).catch((e) => {
