...
- **Forming-pair hold winners (2026-08-30)**: Aug ON v6 entered the TEAM/TSLA/AAPL longs then cut them with day-trade fuses (`max_loss` at −2%, `RUNNER_TOP_FORMATION_1H` flatten at +3–4%) before Trend-Hold's 5% MFE promote. `shouldDeferFormingPairExit` applies the Trend-Hold *lesson* to forming-pair LONGs from bar 1 while HTF is formed/forming and LTF is not dump-broken: defer those fuses plus doctrine / thesis flip / parked daily `ema_regime_reversed` / Trend-Hold giveback set. 1H runner-top trims instead of flattening. Catastrophe floor stays −6%. Crown-jewel exits (`PHASE_LEAVE*`, `SOFT_FUSE*`, `TD_EXHAUSTION*`) still fire. Identity is the ticket `entry_path`, never the live card. Preprod Aug ON v7: TEAM Aug 3/7 same-day `max_loss` gone (held, 85–90% trim into the rip, leftover SL); TSLA Aug 13 `PHASE_LEAVE` still +$285; Aug 24 `max_loss` still fired (structure said no). Do not enable global `deep_audit_trend_hold_enabled` from this. Flag: `deep_audit_forming_pair_hold_winners` (default ON).
