...
- **Broker Connections page feeds (2026-08-13)**: day timeline = `/timed/broker/day-actions` (model `account_ledger` × bridge per-account fills/rejects × client-ring skips; ops mirror `/timed/admin/broker-bridge/day-actions`); positions = bridge join + worker price/P&L enrichment (Pro gate, getDailyChange aliases) + model-open syncable rows; scoped sync = `POST /timed/broker/sync-position` (single-account via `broker_account_id`, RTH + flat + cooldown guards).
