...
describe("analyzeTickerForWeekendDesk", () => {
  it("promotes a retest + ST magnet + elite into a Timed Uptick", () => {
    const out = card({
      ticker: "CRDO",
      rank: 78,
  ...
  it("keeps a lone approaching trendline as watch, not a Timed Uptick", () => {
    const out = card({
      ticker: "XYZ",
  ...
  it("notes Newton's list as overlap, not the Timed Uptick definition", () => {
...
describe("composeWeekendDesk", () => {
  it("splits confluence Upticks from single-tag watches and outside-book promotions", () => {
    const crdo = card({
      ticker: "CRDO",
      flags: {
        breakout_retest: true,
  ...
  it("email copy has no second person", () => {
    const desk = composeWeekendDesk({
      cards: [card({
        ticker: "CRDO",
    ...
    const html = renderWeekendDeskHtml(desk);
    ...
    expect(text).toContain("TIMED UPTICKS");
  });
});
