...
- **Weekend CMT Desk / TT Setups (2026-09-12):** `*/5` scoring skips outside OH. Saturday 10:00 ET (tt-research hourly) pages a full-universe rescore, then composes a short unique list (named support/resistance, daily-brief chips, candle charts with drawn trendlines). Also-on-tape names get charts too. Longs lead; quality shorts only when longs are thin. Fired / magnet copy names the structural target (magnet shelf, or a nearby handle on a fired break) and R:R when both sides are real — not a distant 150/300/500 mark. Daily/weekly chart-image collapses 00:00+04:00 UTC duplicate session stamps (AMAT June/July). Opposite-side magnets that are not nearby stay off the copy. Email is **admin-only** until `WEEKEND_DESK_BROADCAST=1`. Not Newton's monthly Upticks, not a buy path. Playbook: `skills/weekend-desk.md`.
