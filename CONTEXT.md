...
- **Forming-pair LONG floors (2026-08-29)**: complementary clocks fired but TEAM/TSLA/AAPL still died on `focus_conviction_below_floor` / `focus_tier_c_below_c_floor` / `h3_rank_below_transitional_floor` / `h3_consensus_below_min` sitting *above* `tt_forming_pair`. `isFormingPairFloorContext` (LONG only) lowers conviction to 40 and bypasses Tier-C / transitional rank / consensus — same shape as reclaim P15. SHORT excluded so TEAM Jul 8 `HTF_BEAR_LTF_BEAR` cannot fade the rip. Kill: `deep_audit_forming_pair_floors=false`. Do not drop the global 60 accumulate floor.
