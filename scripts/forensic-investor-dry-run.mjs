...
function renderMarkdown(s, byTicker, perTrade) {
  ...
  lines.push(``);
  lines.push(`## Verdict (post Phase 3.9d tuning)`);
  lines.push(``);
  lines.push(`Phase 3.9d landed the **strong-score → accumulate threshold lowered from 70 → 65** (config-driven via \`deep_audit_investor_accumulate_strong_score_min\`). Re-running the same forensic against canonical Phase C trader entries:`);
  lines.push(``);
  lines.push(`| metric | pre-tuning (default 70) | post-tuning (default 65) | Δ |`);
  lines.push(`|---|---:|---:|---:|`);
  lines.push(`| accumulate | 104 (20.1%) | **${s.totals.accumulate} (${s.totals.accumulate_pct}%)** | +${s.totals.accumulate - 104} (${(s.totals.accumulate / 104).toFixed(1)}×) |`);
  lines.push(`| watch | 358 (69.2%) | ${s.totals.watch} (${s.totals.watch_pct}%) | ${s.totals.watch - 358} |`);
  lines.push(`| research_* | 55 | ${s.totals.research_total} | ${s.totals.research_total - 55} |`);
  lines.push(``);
  lines.push(`**Per-ticker Δ on the blueprint cohort:**`);
  lines.push(``);
  lines.push(`- **PLTR: 0 → 13** of 49 entries (was 0%, now 27%)`);
  lines.push(`- **TSM: 0 → 10** of 24 (now 42%)`);
  lines.push(`- **AMD: 8 → 29** of 39 (now 74%)`);
  lines.push(`- **AEHR: 13 → 33** of 37 (now 89%)`);
  lines.push(`- **BE: 40 → 46** of 52 (now 88%)`);
  lines.push(`- **GEV: 10 → 31** of 52 (now 60%)`);
  lines.push(`- **GOOGL: 10 → 29** of 72 (now 40%)`);
  lines.push(``);
  lines.push(`**SNDK remains the outlier**: only 8 of 41 entries → accumulate (was 6). The limiting factor is its low average score (47.8), not the threshold. SNDK's score-component breakdown is dominated by its low \`accumulationSignal\` and \`monthlyTrend\` contributions — addressed in the follow-up Phase 3.9e (tune \`detectAccumulationZone\`).`);
  lines.push(``);
  lines.push(`**Remaining tuning opportunity** (Phase 3.9e — separate PR):`);
  lines.push(``);
  lines.push(`- **\`detectAccumulationZone\` averages 0.4 of 15 possible contribution** — almost dead weight on momentum-runner cohorts. Tuning it to recognize "above weekly EMA21 + within 10% of 50-day high + monthly bull" would add 5-10 pts to cohort score, capturing the SNDK-class trades whose component scoring drags below 65.`);
  lines.push(``);
  lines.push(`## Headline`);
  lines.push(``);
