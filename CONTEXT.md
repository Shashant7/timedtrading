...
- **A cross-tenant fan-out makes every later reduce look under-sized (2026-09-22)**: NBIS opened 3 shares over two Webull accounts (1 partner cash, 2 owner Roth), so the entry ratio was `3/9.98335901386749` and the exit's 2 shares paged `expected 3.0000 at entry ratio` hourly. Nothing was under-sold — `clampExitOpsToHoldings` budgets against `loadBrokerHeldEquity`, which covers only the OWNER's accounts. `computeTradeRelativeQty` now forgives a reduce *shortfall* when the owner holds none of the ticker (the residual is already carried as `sync_state: broker_orphan`), and still pages an over-sell.
