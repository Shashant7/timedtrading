...
function AutopsyModal({
  ...
  const rawEntry = Number(trade?.entry_price ?? trade?.entryPrice);
  const rawExit = Number(trade?.exit_price ?? trade?.exitPrice);
  const effectiveEntry = Number(trade?.effective_entry_price);
  const effectiveExit = Number(trade?.effective_exit_price);
  const displayEntry = Number.isFinite(effectiveEntry) ? effectiveEntry : rawEntry;
  const displayExit = Number.isFinite(effectiveExit) ? effectiveExit : rawExit;
  ...
  }, "raw ", fmtUsd(rawEntry))), React.createElement("div", {
  ...
  }, "raw ", fmtUsd(rawExit)), effectiveExitBasis && React.createElement("span", {
    className: "text-[8px] text-[#9ca3af] uppercase"
  }, "(", effectiveExitBasis.replace(/_/g, " "), ")")), React.createElement("div", {
    className: "flex items-center gap-1 px-2 py-0.5 rounded",
    style: {
      background: Number(trade?.pnl) >= 0 ? "rgba(34,197,94,0.08)" : "rgba(239,68,72,0.08)",
      border: `1px solid ${Number(trade?.pnl) >= 0 ? "rgba(34,197,94,0.2)" : "rgba(239,68,72,0.2)"}`
    }
  }, React.createElement("span", {
  ...
  }, fmtUsd(trade?.pnl)), hasRealizedTrimCarry && React.createElement("span", {
    className: "text-[8px] text-[#9ca3af] uppercase"
  }, "includes realized trim")), (() => {
    const _sh = Number(trade?.shares);
    if (!_sh || !Number.isFinite(_sh) || _sh <= 0) return null;
    const _trimPct = Number(trade?.trimmed_pct || trade?.trimmedPct || 0);
  ...
  })(), trade?.status && React.createElement("span", {
    className: "px-1.5 py-0.5 rounded text-[9px] font-bold uppercase",
    style: {
      background: trade.status === "WIN" ? "#22c55e1a" : trade.status === "LOSS" ? "#ef44441a" : "#9ca3af1a",
      color: trade.status === "WIN" ? "#22c55e" : trade.status === "LOSS" ? "#ef4444" : "#9ca3af"
    }
  }, trade.status), React.createElement("span", {
...
function App({
  ...
  const refreshAll = useCallback(async () => {
    ...
    try {
      ...
      if (tradesRes.ok && Array.isArray(tradesRes.trades)) setTrades(tradesRes.trades);else setError(tradesRes?.error === "unauthorized" ? "Admin access required" : "Failed to load trades");
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        if (tradesData.ok && Array.isArray(tradesData.trades)) setTrades(tradesData.trades);else setError(tradesData?.error === "unauthorized" ? "Admin access required" : "Failed to load trades");
  ...
  return React.createElement("div", {
    ...
    className: "px-3 py-2 sm:py-1.5 rounded-lg bg-white/[0.04] border border-white/[0.06] text-[13px] text-white col-span-1"
  })), (filterTicker || filterClassification || filterDateFrom || filterDateTo || filterRunId !== RUN_FILTER_LATEST) && React.createElement("button", {
    onClick: () => {
      setFilterTicker("");
      setFilterClassification("");
      setFilterDateFrom("");
      setFilterDateTo("");
      setFilterRunId(RUN_FILTER_LATEST);
    },
    className: "px-3 py-2 sm:py-1.5 text-[12px] text-[#6b7280] hover:text-white col-span-2 sm:col-span-1"
  }, "Clear filters"), filteredTrades.length !== trades.length && React.createElement("span", {
    className: "text-[12px] text-[#6b7280] self-center col-span-2 sm:col-span-1"
  }, "Showing ", filteredTrades.length, " of ", trades.length, filterRunId === RUN_FILTER_LATEST && runMeta.latestRunId ? ` · latest completed: ${runMeta.latestRunId}` : "", filterRunId === RUN_FILTER_LIVE ? ` · live replay${runMeta.liveRunId ? `: ${runMeta.liveRunId}` : ""}` : "")), loading && React.createElement("div", {
    ...
    onClick: () => setFilterRunId(RUN_FILTER_LIVE)
  }, "View live trades"), " above to watch its progress.")), !loading && !error && trades.length > 0 && filteredTrades.length === 0 && React.createElement("div", {
    className: "tt-card p-12 text-center text-[#6b7280]"
  }, "No trades match the current filters."), !loading && !error && filteredTrades.length > 0 && React.createElement(React.Fragment, null, React.createElement("div", {
    ...
    className: "px-4 py-3 text-left font-semibold text-[#6b7280]"
  }, "Classification"))), React.createElement("tbody", null, filteredTrades.map((t, i) => {
