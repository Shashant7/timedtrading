#!/usr/bin/env node
/**
 * Push July v12 config — PRE-PROD ONLY.
 *
 * v12 = v6 challenger baseline + anchor selectivity restores:
 *   - deep_audit_index_model_enabled=false (no dedicated index model entries)
 *   - deep_audit_focus_min_entry_conviction=70 (revert v10/v11 74 — ineffective)
 *   - deep_audit_pullback_non_prime_min_rank=90 (anchor used rank≥90 for 84% WR)
 *
 * Worker fix (same PR): SPY/QQQ/IWM stock paths blocked even when index model OFF.
 * v11 proved index_model_enabled=false alone still admitted 15 index trades via
 * tt_ath_breakout / tt_pullback / tt_n_test_support.
 *
 * Usage: TIMED_API_KEY=... node scripts/push-july-v12-config.mjs [--dry-run]
 */
...
async function main() {
  console.log(`Pushing v12 config (${UPDATES.length} keys) to PRE-PROD ONLY: ${PREPROD}`);
