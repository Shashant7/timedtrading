...
function MissionControl({
  ...
  const triggerBackfill = useCallback(async tf => {
    if (backfillBusy) return;
    const UNIVERSE_MAX = 280;
    const BATCH_SIZE = 20;
    const TOTAL_BATCHES = Math.ceil(UNIVERSE_MAX / BATCH_SIZE);
    if (!confirm(`Trigger ${tf} backfill for the full universe (canonical + user-added) covering the last 35 days?\n\n` + `Will run in ${TOTAL_BATCHES} batches of ${BATCH_SIZE} tickers each.\n` + `Each batch takes 10-25 seconds. Total ${tf} backfill ≈ ${Math.round(TOTAL_BATCHES * 0.3)}-${Math.round(TOTAL_BATCHES * 0.5)} minutes.\n\n` + `Progress will tick after every batch. Do not refresh the page.`)) return;
    setBackfillBusy(tf);
    setBackfillMsg(`Backfill ${tf} starting (batch 0/${TOTAL_BATCHES})…`);
    const start = Date.now();
    let totalUpserted = 0,
      totalErrors = 0,
      totalTickers = 0,
      batchesCompleted = 0;
    try {
      for (let offset = 0; offset < UNIVERSE_MAX; offset += BATCH_SIZE) {
        const batchIdx = Math.floor(offset / BATCH_SIZE) + 1;
        const elapsedSec = Math.floor((Date.now() - start) / 1000);
        setBackfillMsg(`Backfill ${tf} batch ${batchIdx}/${TOTAL_BATCHES}… (${totalUpserted} upserted so far, ${elapsedSec}s elapsed)`);
        const url = `${API_BASE}/timed/admin/alpaca-backfill?tf=${tf}&sinceDays=35&include_user=1&offset=${offset}&limit=${BATCH_SIZE}`;
        try {
          ...
          if (!res.ok) {
            console.warn(`[BACKFILL] batch ${batchIdx} HTTP ${res.status}`);
            totalErrors++;
            continue;
          }
          const j = await res.json().catch(() => ({}));
          if (j.ok) {
            totalUpserted += Number(j.upserted) || 0;
            totalErrors += Number(j.errors) || 0;
            totalTickers += Number(j.tickers) || 0;
            batchesCompleted++;
            if ((Number(j.tickers) || 0) === 0) break;
          } else {
            console.warn(`[BACKFILL] batch ${batchIdx} failed:`, j.error);
            totalErrors++;
          }
        } catch (be) {
          console.warn(`[BACKFILL] batch ${batchIdx} threw:`, String(be.message || be));
          totalErrors++;
        }
      }
      const elapsed = Math.floor((Date.now() - start) / 1000);
      const ok = batchesCompleted > 0 && totalUpserted > 0;
      const msg = ok ? `✓ Backfill ${tf} done in ${elapsed}s — ${totalUpserted} candles upserted across ${totalTickers} tickers (${batchesCompleted} batches, ${totalErrors} errors).` : `✗ Backfill ${tf} finished in ${elapsed}s with no upserts — ${totalErrors} batch errors. Check worker logs.`;
      setBackfillMsg(msg);
      ...
      const elapsed = Math.floor((Date.now() - start) / 1000);
      setBackfillMsg(`✗ Backfill ${tf} crashed after ${elapsed}s: ${String(e.message || e)}`);
    } finally {
