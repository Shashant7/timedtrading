...
- **Mobile Tab Nav missing / floating**: never translate Tab Nav for “URL bar” via `innerHeight - visualViewport` — that floats the bar mid-page. Pin `bottom:0` + safe-area; hide only when a text input is focused; bump `SHELL_VERSION` when shipping.
