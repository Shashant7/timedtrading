...
- **FOLLOW-UP: Zero SHORT trades in replay**: Jul–Sep 2025 replay produced 167 trades, all LONG. The SHORT gates (regime-based blocking via `shortsAllowed`, RVOL minimums `shortRvolMin`, 10m EMA gate) are effectively preventing all short entries. Evaluate after calibration: are these gates too restrictive? In a bear market we'd be completely sidelined. Consider: (1) loosening SHORT RVOL from 1.0 to 0.8, (2) allowing SHORTs in TRANSITIONAL regime, (3) adding a dedicated SHORT breakout path (breakdown detection). [2026-03-02]
- **FOLLOW-UP: Low ticker diversity — 36 out of 274 tickers traded**: Only 13% of the universe generated trades during Jul–Sep replay. TJX/WMT/AMZN/MNST dominate. Entry gates (rank minimum, completion cap, fuel exhaustion, RR floor, entry quality score, regime gates) are very selective. Evaluate after calibration: (1) which gates are rejecting the most entries? (2) are the rejected entries actually bad, or are we over-filtering? Run `diagnose-missed-moves` with breakout path analysis. The calibration learning loop should progressively tune gate thresholds — if we're over-filtering, `calibrated_rank_min` and `adaptive_entry_gates` will loosen over cycles. [2026-03-02]
