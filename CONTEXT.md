...
- **Auto catch-up can buy+sell the same fractional lot (2026-07-30)**: First `catchup_auto_rth` run mirrored unmatched DCA_BUY then PRE_FOMC SELL for CRS/CW/NVDA in one pass → Webull buy+sell same qty, no Discord/email. Planner treated ring `side=trim` as not covering SELL lots (re-fire risk). Fix: trim↔sell alias, suppress offsetting buys, `BROKER_CATCHUP_AUTO_RTH` default off, Discord summary when enabled.
