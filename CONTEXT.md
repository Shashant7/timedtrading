...
- **ST / LETF entry must reach the broker (2026-09-03)**: UDOW + TQQQ + TJX wrote the model book and never filled. Cash scale `Math.floor` rejected a tight Roth fractional; `forwardOrderToBridge` treated HTTP 200 without `ok:true` as a fill. Fix the live path: `scaleQtyForCeiling({fractional})`, `bridgeResponseIsOk`, buy-side ring requires an order id. Do not backfill leftover books. Do not auto-enable Individual. Skip SHORT on IRA. Partner does not manually exit names that never filled (`no_manifest_for_trade`).
