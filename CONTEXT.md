...
- **Index-trend STOP/EXIT must flatten the mirrored remainder (2026-09-10)**: paper can Discord + close, then the isolate dies before `/bridge/order` (TQQQ W36 STOP 10:00 ET, qty=0 `not_mirrored`; UDOW W36 still `shares_remaining: 20` after the Sep 4 19:01 ET skip). Closed books never re-enter `options/all`. `healStrandedIndexTrendCloses` on the monolith `*/5` intent drain + `POST /timed/admin/index-trend/heal-closes`. Action tape records `close_qty`, not post-close 0. Do not backfill leftover ENTRIES.
