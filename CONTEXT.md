...
- **Forming-pair hold winners (2026-08-30)**: Aug ON v6 entered the TEAM/TSLA/AAPL longs then cut them with day-trade fuses (`max_loss` at −2%, `RUNNER_TOP_FORMATION_1H` flatten at +3–4%) before Trend-Hold's 5% MFE promote. `shouldDeferFormingPairExit` applies the Trend-Hold *lesson* to forming-pair LONGs from bar 1 while HTF is formed/forming. LTF dump-broken only flattens when the daily 21 is also lost (AAPL June). TEAM Aug 24–25 printed LTF −24 on a −2.3% shake while HTF was +33 and the 21 was +8% — that is a shake, not a dump. 1H runner-top trims instead of flattening. Catastrophe floor stays −6%. Crown-jewel exits (`PHASE_LEAVE*`, `SOFT_FUSE*`, `TD_EXHAUSTION*`) still fire. Identity is the ticket `entry_path`. Do not enable global `deep_audit_trend_hold_enabled`. Flag: `deep_audit_forming_pair_hold_winners` (default ON).
